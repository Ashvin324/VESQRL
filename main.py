...
def run():
    ...
    log_dir = os.path.join(
        'logs', args.env_id,
        f'vesqrl-{datetime.now().strftime("%Y%m%d-%H%M")}')

    agent = VESQRL(env=env, env1=env1, env_name=args.env_id, log_dir=log_dir, **configs, min_epsilon=0.25)
    agent.pretrain()
    agent.train()
