...
def states_filter(states,horizons,max_horizon):
    #horizons[horizons>max_horizon]=max_horizon
    index=torch.where(horizons>0)[0]
    states=torch.index_select(states,0,index)
    horizons=torch.index_select(horizons,0,index)
    #print(states.size())
    #print(horizons.size())
    return states,horizons
